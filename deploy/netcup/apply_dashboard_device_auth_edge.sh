...
[[ "$production_sha" =~ ^[0-9a-f]{40}$ ]] || { echo "invalid production sha" >&2; exit 3; }
[[ "$staging_sha" =~ ^[0-9a-f]{40}$ ]] || { echo "invalid staging sha" >&2; exit 4; }
...
for c in "$PROD_N8N" "$PROD_OLLAMA" "$STAGING_N8N" "$STAGING_OLLAMA" "$EDGE"; do
...
[[ -f "$CADDY_SOURCE" && -r "$CADDY_SOURCE" && -w "$CADDY_SOURCE" ]] || { echo "Caddy host source not writable" >&2; exit 15; }
...
before_hash_count="$(grep -Ec '^[[:space:]]+[A-Za-z0-9._@-]+[[:space:]]+\$2[aby]\$' "$CADDY_SOURCE" || true)"
[[ "$before_hash_count" -ge 2 ]] || { echo "could not prove existing password hash entries" >&2; exit 17; }
...
def find_site(host):
    ...
    for i,line in enumerate(lines):
        if line.strip()==f"{host} {{": start=i; break
    if start is None: raise SystemExit(f"missing site block: {host}")
    depth=0
    for j in range(start,len(lines)):
        depth += lines[j].count("{")-lines[j].count("}")
        if j>start and depth==0: return start,j
    raise SystemExit(f"unterminated site block: {host}")

def auth_body(block):
    start=None
    for i,line in enumerate(block):
        if line.strip()=="basic_auth {": start=i; break
    if start is None: raise SystemExit("basic_auth block missing")
    depth=0
    for j in range(start,len(block)):
        depth += block[j].count("{")-block[j].count("}")
        if j>start and depth==0:
            body=[x.strip() for x in block[start+1:j] if x.strip()]
            if not body: raise SystemExit("basic_auth body empty")
            return body
...
def render(host, fastapi, streamlit, old):
    ...
    if "@device_login path /_munshi-auth/login" in joined:
        return old
    if joined.count("basic_auth {")!=1: raise SystemExit(f"{host}: unexpected basic_auth count")
    auth=auth_body(old)
    return [
        f"{host} {{\n","    encode zstd gzip\n","\n",
...
for host,(fastapi,streamlit) in sites.items():
    s,e=find_site(host); repls.append((s,e,render(host,fastapi,streamlit,lines[s:e+1])))
out=lines[:]
for s,e,new in sorted(repls, reverse=True): out[s:e+1]=new
text="".join(out)
for host,(fastapi,streamlit) in sites.items():
    if text.count(f"{host} {{")!=1: raise SystemExit(f"{host}: bad site block count")
    if f"forward_auth 127.0.0.1:{fastapi}" not in text: raise SystemExit(f"{host}: forward_auth missing")
    if f"reverse_proxy 127.0.0.1:{streamlit}" not in text: raise SystemExit(f"{host}: streamlit proxy missing")
path.write_text(text,encoding="utf-8")
PY

after_hash_count="$(grep -Ec '^[[:space:]]+[A-Za-z0-9._@-]+[[:space:]]+\$2[aby]\$' "$candidate" || true)"
[[ "$after_hash_count" == "$before_hash_count" ]] || { echo "password hash entry count changed" >&2; exit 18; }
