...
"$REPO/scripts/netcup/verify_production_runtime_contract.sh"
...
old_hunter_id="$(docker inspect -f '{{.Id}}' "$H")"
...
rollback() {
  ...
  "$REPO/scripts/netcup/verify_production_runtime_contract.sh" || true
...
active_workers="$(
  docker exec "$H" sh -lc '
    ps -eo args= |
      grep -E "[a]pp\.stored_job_n8n_worker|[a]pp\.manual_input_worker" |
      wc -l | tr -d " "
  '
)"
...
explicit_locks="$(
  docker logs --timestamps --since 10m "$H" 2>&1 |
  grep -Ei 'database is locked|database table is locked|database schema is locked|OperationalError:.*locked' || true
)"
...
try:
    n=db.execute("""
      ...
      WHERE status IN ('new','running','waiting') OR stoppedAt IS NULL
    """).fetchone()[0]
except Exception:
    n=db.execute("""
      SELECT COUNT(*) FROM execution_entity
...
n=db.execute(
    f'SELECT COUNT(*) FROM n8n_dispatch_queue WHERE "{state}" IN ({ph})', states
).fetchone()[0]
print(n)
db.close()
PY
...
git merge-base --is-ancestor "$commit" "origin/$branch" || {
  echo "requested SHA is not contained in origin/$branch" >&2
  exit 24
}
